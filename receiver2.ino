...
// FFT_config_t objects for detection
fft_config_t *plan_pixie;
fft_config_t *plan_leviosa;
fft_config_t *plan_lumos;
...
float fft_output_pixie[FFT_N];  // FFT output buffer
...
void processFFT(fft_config_t *plan, float fft_return[2]) {
  ...
  for (int k = 1; k < plan->size / 2; k++) {
    float real = plan->output[2*k];
    float imag = plan->output[2*k+1];
...
void setup() {
  ...
  plan_pixie   = fft_init(FFT_N, FFT_REAL, FFT_FORWARD, fft_input_pixie, fft_output_pixie);
  plan_leviosa = fft_init(FFT_N, FFT_REAL, FFT_FORWARD, fft_input_leviosa, fft_output_leviosa);
  plan_lumos   = fft_init(FFT_N, FFT_REAL, FFT_FORWARD, fft_input_lumos, fft_output_lumos);
...
void loop() {
  ...
  if (currentMicros - previousMicros >= sampleInterval) {
    ...
    if (sampleIndex_pixie >= FFT_N) {
      float fft_return_pixie[2];
      processFFT(plan_pixie, fft_return_pixie);
      sampleIndex_pixie = 0;  // Reset the sample index for the next batch
      float fundamental_freq = fft_return_pixie[0];
      float max_magnitude = fft_return_pixie[1];
      ...
      if (sampleIndex_leviosa >= FFT_N) {
        ...
        processFFT(plan_leviosa, fft_return_lev);
    ...
    if (sampleIndex_lumos >= FFT_N) {
      ...
      processFFT(plan_lumos, fft_return_lumos);
      sampleIndex_lumos = 0;
